...
description: >-
  Use for any dbt work - building or modifying models, debugging errors, exploring
  unfamiliar data sources, writing tests, or evaluating the impact of changes. Builds
  and modifies dbt models, writes SQL transformations using ref() and source(), creates
  tests, and validates results with dbt show.
...
- Querying the dbt Semantic Layer (out of scope for this skill)
...
### House rules for incremental anti-joins

- **`NOT EXISTS`, never `NOT IN`.** NOT IN's three-valued logic means one NULL in the
  subquery makes every `x NOT IN (...)` UNKNOWN -> the model **silently stops inserting
  anything new, forever, with every test green**. Nothing structurally prevents the
  poison row on warehouses that don't enforce NOT NULL. NOT EXISTS is NULL-safe with the
  same plan or better - there is no upside to NOT IN.
- **For append-only event models, the anti-join key must be the FULL event identity**
  (entity, time, *and* state/value) - and the model's uniqueness test must match it
  exactly; if the two disagree, one of them is wrong. Deduplicating on (entity, time)
  alone conflates distinct states at the same instant: a backdated event matching an
  existing row's timestamp is silently dropped. (Sources demonstrably bulk-mutate
  timestamp columns - backdating is plausible, not theoretical.)

Both failure shapes are latent: they pass every build verification while current data is
clean. Any diff touching incremental predicates warrants the data-integrity and SCD
reviewer personas (`review-changes`).
...
| `NOT IN` for incremental anti-joins | `NOT EXISTS` - one NULL silently stops all future inserts |
| Anti-join keyed on a subset of event identity | Key on the full (entity, time, state) identity, matched by the uniqueness test |
