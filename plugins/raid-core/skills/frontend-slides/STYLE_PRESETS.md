...
**Default for any deck made on a Rebtech engagement** -- workshops, steering groups, handovers, proposals. Use it without a style-discovery round unless the user asks for alternatives, or the deck is going out under the customer's own brand (then use theirs).

**Vibe:** Calm, premium, understated confidence. Warm neutrals on near-black; gold is the only accent.

**Layout:** Dark title and section slides on the ink gradient; content slides on cream with ink text. Left-aligned, with a small uppercase mono kicker above each title. The density mode decides how much goes on a slide: speaker-led gets one idea and generous negative space, reading-first gets tables, stat grids and diagrams -- the palette, type and rules below stay the same either way. Logo top-left on title and closing slides, not on every slide; where `html-template.md` says otherwise, this preset wins.

**Typography:**
- Display + body: `Geist` (500 for titles, 400 body), tight tracking on titles (`-0.025em`), `line-height: 1.05`
- Labels, kickers, page numbers, stats: `Geist Mono` (600), uppercase, `letter-spacing: 0.06em`; stats use `font-variant-numeric: tabular-nums`
...
:root {
    --ink: #000000;
    --ink-2: #14110d;
    --ink-gradient: linear-gradient(135deg, #14110d 0%, #000000 70%);
    --brown: #5E503F;   /* muted text on cream, rules, secondary data series */
    --gold: #C6AC83;    /* the one accent: kickers, key numbers, highlights, primary data series */
    --sand: #E0D4C1;    /* cards and table stripes on cream */
    --cream: #E9E1D5;   /* light slide background */
    --stage-bg: #000000;
}
```

**Logo:** [assets/rebtech-logo.svg](assets/rebtech-logo.svg). Inline the SVG (the deck is one self-contained file). The wordmark is `fill="currentColor"` and the dots stay gold, so set `color: #fff` on dark slides and `color: var(--ink)` on cream. Never recolor the dots, stretch it, or put it on a busy background.

**Signature Elements:**
- Gold as a scalpel, not a paint roller: on cream, gold marks the one thing to look at (a key number, the current step) and kickers stay `--brown`; on ink, kickers may be gold
- Big mono numbers with a small uppercase label under them for KPI and stat slides
- Thin `1px` rules in `--brown` (on cream) or `rgba(198,172,131,0.25)` (on ink) to separate content, rather than cards
- Diagrams (architecture, flows): square-cornered boxes filled `--sand` with a `1px` `--brown` border, the emphasised box in `--ink`, and thin arrows
- Data charts in gold, brown and sand only; no rainbow palettes

**Avoid:** emoji anywhere, including the edit toggle (`html-template.md` shows a pencil emoji -- use the text "Edit"), gradients other than the ink gradient, drop shadows, rounded "SaaS" cards, stock icons, and more than one accent color.
