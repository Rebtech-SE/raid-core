...
description: "Use when sequencing an addition, refactor or rewrite. Remove the dead weight first -- unread columns, unused models, redundant guards -- then build on the simpler base."
...
**In a warehouse the dead weight is usually columns and models.** Before extending a model,
drop the columns nothing reads, the intermediate models nothing selects from, and the tests
that assert what the schema already guarantees. A `SELECT *` carried through four layers is
four layers of scan cost defending columns no report has ever used. Check lineage before
you assume something is load-bearing -- and check it, don't guess.
