...
create_worktree() {
  ...
  # Propagate ONLY the base `.env` (the local/dev connection env a measurement run needs).
  # Do NOT copy every `.env*`: named-environment files like `.env.production` routinely
  # hold real warehouse/cloud credentials, and these worktrees are handed to autonomous
  # experiment workers and may be left around for resume/debugging -- copying production
  # secrets into them by default is an unnecessary credential-spread risk. Any additional
  # env file an experiment genuinely needs must be opted in explicitly via the spec's
  # `parallel.shared_files` (handled in the shared-files loop below).
  if [[ -f "$GIT_ROOT/.env" ]]; then
    cp "$GIT_ROOT/.env" "$worktree_path/.env"
  fi
  # Surface any skipped named-environment files so the operator knows they were NOT copied.
  for f in "$GIT_ROOT"/.env.*; do
    ...
    case "$envbase" in
      .env.example) : ;;
      *) echo -e "${YELLOW}Note: not copying $envbase into the experiment worktree (only base .env is propagated). Add it to parallel.shared_files if an experiment needs it.${NC}" >&2 ;;
    esac
  done

  # Copy shared files. These paths come from the optimization spec, so reject anything
  # that could escape the repo (source) or the worktree (destination) before the
  # destructive `rm -rf`/`cp` below -- an entry like `../../x` must not delete or read
  # outside the intended trees.
  for shared_file in "$@"; do
    if [[ "$shared_file" = /* ]] || [[ "$shared_file" == *..* ]]; then
      echo -e "${RED}Error: shared_file must be a repo-relative path without '..': $shared_file${NC}" >&2
      return 1
    fi

    local src="$GIT_ROOT/$shared_file"
    local dest="$worktree_path/$shared_file"

    # Resolve the destination's parent and confirm it stays under the worktree before
    # writing. (The source is already constrained by the no-absolute/no-`..` check above.)
    local dest_parent
    dest_parent=$(dirname "$dest")
    mkdir -p "$dest_parent"
    local dest_parent_real worktree_real
    dest_parent_real=$(cd "$dest_parent" && pwd -P) || return 1
    worktree_real=$(cd "$worktree_path" && pwd -P) || return 1
    case "$dest_parent_real/" in
      "$worktree_real"/*) : ;;
      *)
        echo -e "${RED}Error: shared_file destination escapes the worktree: $shared_file${NC}" >&2
        return 1
        ;;
    esac
