...
name: tune-workload
description: >-
  Use when the user says 'this query/model is too slow', 'cut the warehouse cost', 'tune
  this pipeline', 'speed up the build', 'improve the DQ pass rate', or for any data
  outcome worth systematic experimentation rather than a one-shot fix. Runs a
  metric-driven tuning loop: define a measurable goal (query runtime, warehouse cost,
  pipeline duration, DQ pass rate), run isolated experiments, measure each against hard
  gates and optionally an LLM-as-judge, and keep only behavior-safe improvements. For a
  known one-line fix, just fix it; for correctness review use review-changes.
argument-hint: "[path to an optimization spec YAML, or describe the data workload and goal]"
---

# RAID Optimize: metric-driven tuning loop

Run a metric-driven tuning loop on a data workload. Define a goal (cost, runtime, freshness,
DQ pass rate), build the measurement harness first, then run isolated experiments -- each a
candidate rewrite of a model, a partition/cluster change, an incremental strategy, a warehouse
sizing, a join order -- that converge toward the best variant while a hard gate guarantees the
...
This skill ships three helper scripts in its own `scripts/` folder: `measure.sh`,
`parallel-probe.sh`, and `experiment-worktree.sh`. Every `bash scripts/...` invocation in
this runbook refers to **these bundled scripts in this skill's directory**, not to any
`scripts/` folder in the engagement repo.

**Run them from the engagement repo root, invoking each by its full path under this
skill's directory.** Resolve this skill's directory once into a shell variable and use it
in every command below:

```bash
SKILL_DIR="<absolute path to this skill's directory>"   # the folder containing this SKILL.md
```

Then the snippets in this runbook use `"$SKILL_DIR"/scripts/...`. Do **not** `cd` into the
skill directory to run them: `experiment-worktree.sh` derives `GIT_ROOT` from `git
rev-parse` in the current directory, and `measure.sh` resolves the measurement
`working_directory` relative to the current directory -- both must see the engagement
repo, not the plugin install location. The scratch state and git worktrees the scripts
create live in the engagement repo.
...
### File Locations (all under `.raid/optimize/<spec-name>/`)
...
2. **Detect whether the optimization target is qualitative or quantitative** — this determines `type: hard` vs `type: judge` and is the single most important spec decision:
   ...
   **Use `type: hard`** when (this is the common case for data work):
   - The metric is a scalar number with a clear "better" direction
   - The metric is objectively measurable from a run: model/query runtime (seconds),
     warehouse cost or slot/DBU/CU-seconds, bytes scanned, pipeline wall-clock,
     incremental-load duration, rows-per-second throughput, dbt test pass rate
   - No human judgment is needed to evaluate "is this result actually good?"
   - Examples: cut a gold model's build time, halve bytes scanned on a dashboard query,
     reduce nightly pipeline duration, lower warehouse cost per run, raise the DQ test
     pass rate
   - **The output-parity gate is almost always a hard gate here:** a degenerate check
     (`rowcount_delta == 0`, `checksum_match == 1`) that proves the tuned model returns
     the same data. Make this gate mandatory so the loop can never "win" by changing results.

   **Use `type: judge`** when:
   - "Better" needs semantic understanding a number can't capture, and exact parity
     isn't the target
   - A human would need to look at the output to say "this is better"
   - Proxy metrics can mislead (e.g., "more DQ rules" does not mean "better DQ coverage";
     "fewer rows flagged" does not mean "cleaner data")
   - Examples: usefulness/precision of generated data-quality rules or anomaly flags,
     realism of synthetic/seed test data, quality of generated metric or column
     descriptions, semantic equivalence of a large rewrite where row-level parity is
   ...
   **IMPORTANT**: If the target is qualitative, **strongly recommend `type: judge`**. Explain that hard metrics alone will optimize proxy numbers without checking actual quality. Show the user the three-tier approach:
   - **Degenerate gates** (hard, cheap, fast): catch obviously broken solutions — e.g., output parity broken (`rowcount_delta != 0`), a dbt test now failing, or "0 rows returned". Run first. If gates fail, skip the expensive judge step.
   ...
   - **Diagnostics** (logged, not gated): distribution stats, row counts, null rates, timing — useful for understanding WHY a judge score changed.
...
3. **Design the sampling strategy** (for `type: judge`):
   ...
   - **What does one "item" look like?** (a generated DQ rule, a flagged anomaly row, a row of synthetic test data, a generated column description, etc.)
   - **What are the natural strata?** (e.g., high-volume vs long-tail entities; rows flagged by the rule vs rows passed; recent vs historical partitions)
   - **Where are quality failures most likely?** (e.g., a rule that flags everything in a skewed partition; an anomaly score that fires only on the largest table; descriptions that are vague on the least-documented columns)
   - **What total sample size balances cost vs signal?** (default: 30 items, adjust based on output volume)

   Example stratified sampling for judging generated data-quality rules:
   ```yaml
   stratification:
     - bucket: "high_volume_tables"  # rules on the biggest tables — check for false-positive floods
       count: 10
     - bucket: "core_entities"       # rules on the central marts — representative usefulness
       count: 10
     - bucket: "long_tail"           # rules on rarely-changed tables — check they aren't noise
       count: 10
   singleton_sample: 15              # tables with NO generated rule — check for missed coverage (false negatives)
   ```

   The sampling strategy is domain-specific. For anomaly detection, strata might be "flagged on core marts", "flagged on staging", "not flagged at all". For synthetic test data, strata might be "common values", "edge/boundary values", "null/sentinel cases".

   **Singleton evaluation is critical when the goal involves coverage** — sampling the items that got NO rule/flag checks whether the system is missing obvious cases.
...
4. **Design the rubric** (for `type: judge`):
   ...
   - Includes supplementary fields that help diagnose issues (e.g., `false_positive_risk`, `business_rule_match`)
   - Is specific enough that two judges would give similar scores
   - Does NOT assume bigger/more is better — "flags 5% of rows" is not inherently good or bad

   Example for judging a generated data-quality rule:
   ```yaml
   rubric: |
     Rate this data-quality rule 1-5:
...
5. Guide the user through the remaining spec fields:
   - What degenerate cases should be rejected? (gates — e.g., "rowcount_delta == 0" and "checksum_match == 1" guarantee output parity, "tests_failed == 0" keeps dbt tests green)
...
6. Write the spec to `.raid/optimize/<spec-name>/spec.yaml`
7. Present the spec to the user for approval before proceeding

### 0.3 Search Prior Learnings

Search the repo's own docs and the issue tracker for prior optimization work on similar topics. If relevant prior work exists, incorporate them into the approach.
...
**If branch exists**, check for an existing experiment log at `.raid/optimize/<spec-name>/experiment-log.yaml`.
...
```bash
mkdir -p .raid/optimize/<spec-name>/
```
...
1. Create the experiment log file at `.raid/optimize/<spec-name>/experiment-log.yaml`
...
Optionally dispatch `raid-repo-research-analyst` for deeper codebase analysis if the scope is large or unfamiliar.
...
7. **IMMEDIATELY append to experiment log on disk (CP-3)** — do not defer this to batch evaluation. Write the experiment entry (iteration, hypothesis, outcome, metrics, learnings) to `.raid/optimize/<spec-name>/experiment-log.yaml` right now. Use the transitional outcome `measured` once the experiment has valid metrics but has not yet been compared to the current best. Update the outcome to `kept`, `reverted`, or another terminal state in the evaluation step, but the raw metrics are on disk and safe from context compaction.
...
4. **Write strategy digest** to `.raid/optimize/<spec-name>/strategy-digest.md`:
...
The experiment log and strategy digest remain in local `.raid/optimize/<spec-name>/` scratch space for resume and audit on this machine only; they do not travel with the branch because that path is gitignored.

Present post-completion options via the platform question tool:

1. **Run `review-changes`** on the cumulative diff (baseline to final). Load the `review-changes` skill on the optimization branch (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
...
2. **Open a PR** via `commit-push-pr` from the optimization branch to `main`, linked to its tracker item if one was supplied.
3. **Continue** with more experiments: re-enter Phase 3 with the current state. State re-read first.
4. **Done** -- leave the optimization branch for manual review.
...
rm -f .raid/optimize/<spec-name>/strategy-digest.md
```
