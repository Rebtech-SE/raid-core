...
3. **Run targeted tests only** for what you changed: a specific test file, a test pattern, or the test you just wrote. Examples: `pytest tests/models/test_orders.py`, `dbt test --select stg_orders`, `dbt build --select model_name`. **Never run the full project test suite** (bare `pytest`, `dbt build` or `dbt test` with no selector) -- the parent runs it once against the combined diff from all fixers. Skip targeted tests for pure doc/comment/string-literal edits with no behavioral impact. If you can't locate targeted tests, note it in `reason` and let the combined run catch any issues.
