---
name: autoreview
description: "Use when the user asks for a code review, second-model review, autoreview, or Codex/Claude/Copilot review, and as a closeout gate after non-trivial edits before commit/ship. Runs one structured single-engine review (Codex, Claude, Amp, Pi, Kimi, or Copilot) over a validated change bundle, then verifies every finding against the real code."
---
...
**RAID addition — the `copilot` engine.** Upstream removed GitHub Copilot because its file tools could not be confined to the reviewed bundle without exposing ignored repository files. RAID re-adds it (`--engine copilot`) by running Copilot in a throwaway sandbox that contains only the review prompt plus a mirror of the changed files, staged at the revision the bundle actually diffs. The mirror is scanned with the same TruffleHog policy as the outgoing prompt and fails closed if the scanner reports findings, errors, or is missing. Nothing outside the sandbox is readable, custom instructions are disabled so the reviewed branch cannot steer its own reviewer, and `--no-tools` is rejected because Copilot needs a read-only file view to load the bundle. Use it when Copilot is the only CLI available; otherwise prefer Codex or Claude, which upstream tests and maintains.
