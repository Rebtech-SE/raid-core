...
- **Missing dead-letter / error routing** -- rows that fail parsing/validation are silently dropped instead of routed to a reject/dead-letter table; a whole batch aborts on one bad record with no quarantine path. The drop is often hidden inside code that *looks* like it quarantines: a valid/invalid split on a predicate that goes NULL for NULL inputs (`filter(cond)` + `filter(NOT cond)` loses those rows from both sides), `mode("DROPMALFORMED")`, an `except: pass` in a per-row transform, or a reject table with no reason, batch id or original record, so nothing in it can be replayed. Every batch should reconcile: rows in = loaded + quarantined. (See the `dead-letter-queue` skill.)
